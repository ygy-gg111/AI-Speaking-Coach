...
当前实现说明：阶段 7 已使用共享 Zustand persist 数据层打通练习完成写入、首页指标、我的场景和月历日期摘要。收藏场景与练习历史已接入 TanStack Query + PostgreSQL API，登录用户同步云端，游客继续保存在浏览器本地；日历数据将在后续沿用同一模式迁移，页面组件和统计函数保持不变。
