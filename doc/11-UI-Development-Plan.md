...
- 复用阶段 2 的场景组件；
- `GET /api/v1/dashboard` 聚合登录用户的学习指标和最近记录；
- 游客继续使用本地记录计算同一套指标。

当前实现说明：Dashboard 已接入 TanStack Query + PostgreSQL API，服务端与游客本地回退复用同一统计函数，并按浏览器时区计算今日分钟、昨日增量和连续学习天数。
...
当前实现说明：阶段 7 已使用共享 Zustand persist 数据层打通练习完成写入、首页指标、我的场景和月历日期摘要。收藏场景、练习历史、月历和日期详情均已接入 TanStack Query + PostgreSQL API，登录用户同步云端，游客继续使用浏览器本地数据；页面组件和统计函数保持不变。
...
当前实现说明：阶段 8 已完成错题本和单词本闭环，以及基于 Prisma、bcrypt、HttpOnly JWT Cookie 的登录、注册、退出登录和用户资料更新。登录用户完成会话后会将核心纠错和自然表达写入 PostgreSQL，错题列表、词汇收藏及复习次数可跨设备恢复，累计复习 3 次自动标记为已掌握；未登录或接口不可用时保留 Zustand 本地体验。单词本已经移除页面硬编码数据，MVP 先沉淀每次 AI 评估给出的纠正表达，后续可扩展为一次对话提取多条短语。页面已补充中英文、响应式和可访问性标注。错误监控已建立供应商无关的结构化日志、敏感字段脱敏、客户端上报接口、Locale 页面错误恢复和全局兜底页；生产环境可在现有传输层继续接入 Sentry 或 OpenTelemetry。
...
当前实现说明：成长报告直接复用共享学习记录和错题数据，通过纯函数生成周期摘要、每日趋势、场景分布、能力估算与待加强分类。登录用户通过 `/api/v1/reports` 从 PostgreSQL 聚合 7 天或 30 天报告和云端错题弱项，游客或接口不可用时自动回退到 Zustand 本地记录。能力值仅使用场景熟练度、纠错频率、新表达和场景覆盖等可验证信号，不包含尚未接入的发音评分，也不生成缺乏总体数据支持的用户排名。
