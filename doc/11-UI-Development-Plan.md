...
当前实现说明：成长报告直接复用共享学习记录和错题数据，通过纯函数生成周期摘要、每日趋势、场景分布、能力估算与待加强分类。登录用户通过 `/api/v1/reports` 从 PostgreSQL 聚合 7 天或 30 天报告，游客或接口不可用时自动回退到 Zustand 本地记录；错题分类独立入库前，云端报告仍使用本地错题补充弱项分布。能力值仅使用场景熟练度、纠错频率、新表达和场景覆盖等可验证信号，不包含尚未接入的发音评分，也不生成缺乏总体数据支持的用户排名。
